...
enum Direction2 { RIGHT2, LEFT2, UP2, DOWN2 };
		...
		if (ev.type == ALLEGRO_EVENT_TIMER) {
			...
			if (state == ALIVE) {
				...
				if (key2[KEY_W]) {
					...
					dy2 = -1;
					dx2 = 0;
					dir2 = UP2;//used for collision later
				}
		...
		if (redraw && al_is_event_queue_empty(event_queue)) {
			...
			if (state == ALIVE) {
				...
				for (int k = 0; k < 50; k++) {
					for (int l = 0; l < 50; l++) {
						if (grid[k][l] == 1)
							al_draw_filled_rectangle(k * 10, l * 10, k * 10 + 10, l * 10 + 10, al_map_rgb(255, 155, 255));
						if (grid[k][l] == 2)
							al_draw_filled_rectangle(k * 10, l * 10, k * 10 + 10, l * 10 + 10, al_map_rgb(0, 25, 255));
					}
				}
				if (lives == 0) {
					state = DEAD;
